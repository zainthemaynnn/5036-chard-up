...
public class Blinkin implements Subsystem  {
    ...
    public static enum BlinkinColor {
        BALANCED(FixedPalette.RAINBOW_PARTY),
        I_WANT_CUBE(Solid.VIOLET),
        I_WANT_CONE(Solid.GOLD),
        RED_ALLIANCE_END(FixedPalette.HEARTBEAT_RED),
        BLUE_ALLIANCE_END(FixedPalette.HEARTBEAT_BLUE),
        RED_ALLIANCE(Solid.RED),
        BLUE_ALLIANCE(Solid.BLUE),
        DEFAULT (Solid.BLUE);
    ...
    public void periodic() {
        if (reading) {
            blinkin.set(SmartDashboard.getNumber("Blinkin", 0.));
        }
    }
}